...
llxHeader('','Secure User Free','');

$form=new Form($db);


// Put here content of your page
print load_fiche_titre('Secure User Free');
...
echo "<br><br>";
echo '<center><table style="width: 50%; float: left;">'."\n";
            echo '<tr class="liste_titre">';
                echo '<td class="liste_titre noborder">User</td>';
                echo '<td class="liste_titre noborder">State</td>';
            echo "</tr>";

$db->begin();
$result = $db->query("SELECT login, pass FROM `llx_user`");
if (!$result)
    echo "Can't load users :'(";
...
while ($i < $num)
{
    ...
    if ($obj)
    {
        if ($i % 2 != 0) $design = "pair"; else $design = "impair";
        echo "<tr class='$design'>";
                echo '<td class="">' . $obj->login . '</td>';
                echo '<td class="">';
                    if ($obj->pass == NULL || $obj->pass == "")
                        echo "<span style='color: green;'>Safe</span>";
                    else   
                    {
                        echo "<span style='color: red;'>Not safe <aside style='cursor: pointer;' onClick=\"if (document.getElementById('pass$i').style.display == 'none') document.getElementById('pass$i').style.display = 'block'; else document.getElementById('pass$i').style.display = 'none' \">Show password</aside><aside id='pass$i' style='display: none; color: black;'>" . $obj->pass. "</aside> </span>";
                        $secure=false;
                    }
                echo '</td>';
            echo "</tr>";
...
if (!$secure)
    echo "<br><br><div class='alert'>Users accounts (password) are not safe. An hacker could show it. <a target='_BLANK' href='https://www.dolistore.com'>Activate SecureUserPro module to incrase safty</a></div>";
else
    echo "<div class='success'>Users accounts are safe.</div>";
