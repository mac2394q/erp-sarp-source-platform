...
function getListOfThirdParties($authentication,$filterthirdparty)
{
    ...
    if (! $error)
    {
        $sql  = "SELECT s.rowid as socRowid, s.nom as ref, s.ref_ext, s.address, s.zip, s.town, c.label as country, s.phone, s.fax, s.email, s.url, s.client, s.fournisseur, s.remise_client, s.fk_stcomm, s.note_public, s.note_private, extra.*";
...
function getClientsGPS($authentication,$latitude,$longitude)
{
	...
	if (! $error)
	{
		$sql = "SELECT fk_soc,latitude,longitude,111.045*DEGREES(ACOS(COS(RADIANS(".$latitude."))*COS(RADIANS(latitude))*COS(RADIANS(".$longitude.")-RADIANS(longitude))+SIN(RADIANS(".$latitude."))*SIN(RADIANS(latitude)))) AS distance FROM ".MAIN_DB_PREFIX."societe_gps ORDER BY distance";
		$resql=$db->query($sql);
		if ($resql)
		{
			$lines=array();
