...
/**
 *      \file       scripts/gpscoord.php
 *        \ingroup    gps
 *      \brief      This file is an example for a command line script to work on GpsCoord
 */
...
// Examples for manipulating class GpsCoord
dol_include_once("/gps/class/gpscoord.class.php");
$gpscoord=new GpsCoord($db);

// Example for inserting creating object in database
/*
dol_syslog($script_file." CREATE", LOG_DEBUG);
$gpscoord->prop1='value_prop1';
$gpscoord->prop2='value_prop2';
$id=$gpscoord->create($user);
if ($id < 0) { $error++; dol_print_error($db,$gpscoord->error); }
...
$result=$gpscoord->fetch($id);
if ($result < 0) { $error; dol_print_error($db,$gpscoord->error); }
else print "Object with id=".$id." loaded\n";
*/

// Example for updating object in database ($gpscoord must have been loaded by a fetch before)
/*
dol_syslog($script_file." UPDATE", LOG_DEBUG);
$gpscoord->prop1='newvalue_prop1';
$gpscoord->prop2='newvalue_prop2';
$result=$gpscoord->update($user);
if ($result < 0) { $error++; dol_print_error($db,$gpscoord->error); }
else print "Object with id ".$gpscoord->id." updated\n";
*/

// Example for deleting object in database ($gpscoord must have been loaded by a fetch before)
/*
dol_syslog($script_file." DELETE", LOG_DEBUG);
$result=$gpscoord->delete($user);
if ($result < 0) { $error++; dol_print_error($db,$gpscoord->error); }
else print "Object with id ".$gpscoord->id." deleted\n";
*/
...
$sql.= " FROM ".MAIN_DB_PREFIX."gpscoord";
