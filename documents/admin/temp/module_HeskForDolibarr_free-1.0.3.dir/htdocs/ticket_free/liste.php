...
/**
 *   	\file       ticket_free/liste.php
 * 		\ingroup   ticket
 * 		\brief      liste les ticket 
...
$id = GETPOST("ticket") ? GETPOST("ticket") : $id;
...
print '<td width=120px>' . $langs->trans('voir les tickets de:') . '</td><td width=120px> ';
//
...
if ($result) {
    ...
    print '<br>' . $langs->trans('rendez vous sur') . '<a href="' . $dol_url_root . '/admin/ticket_free.php">';
...
while ($i < $num) {
    ...
    if (($allticket[$i]['status'] != 3) && ($soap->check_view_ticket($allticket[$i]))) {
        ...
        $var = !$var;
                        ...
                        //ticket
                        print '<tr '.$bc[$var].'><td ><p class="' . $options_priority[$allticket[$i]['priority']] . '"><a href="ticket.php?id=' . $allticket[$i]['id'] . '">' . $allticket[$i]['trackid'] . '</a></p>';
    ...
    print '<script type="text/javascript" charset="utf-8">
			jQuery(document).ready(function() {
				jQuery(\'#ticket\').dataTable({
                                  "oLanguage": {
			"sUrl": "./js/dataTables.'.$langs->trans("LangDatatable").'.txt"
		}
